...
for ix=1:length(SNR)
    ...
    rhos=SNR(ix);
    for k=1:LITER        
        h1=sqrt(deltalsq/2).*(rand(1)+1j*rar (1)+1j*randn(1)); %Channel fading coefficient from base station to user 1        
    ...
    phi3=1+((R1*2*delta2sq)/(a1* delta1sq));
    Pout1_theory(ix)=1-((exp(-R1/(a1*rhos*deltalsa)/(1+((R1*a2*delta2sq)/(al*delta1sq))));
